...
- 性能优化：后端采用 MongoDB 聚合运算、按需加载，前端减少非必要请求，保障大数据量下的响应速度
- 备份与恢复：支持全量数据的手动/自动备份与恢复，完美兼容本地文件和 MongoDB 数据库的双向数据迁移
...
- ✅ 性能优化：仪表盘按需加载与服务端聚合运算
- ✅ 数据备份与恢复：支持手动导出/导入、定时自动备份、历史版本管理、Cron 表达式本地化预览、跨存储模式无缝迁移
...
- 数据可视化
- 真实数据统计
- 数据备份与恢复系统（含自动备份与Cron预览）
- 仪表盘性能大幅优化（后端使用MongoDB聚合运算，解决大数据量下的OOM问题）
